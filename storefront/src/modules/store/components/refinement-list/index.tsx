...
const FilterSection = ({ title, options }: { title: string; options: string[] }) => {
  ...
  return (
    <div className="border-b border-gray-200 py-4 last:border-0">
      <button
        className="flex items-center justify-between w-full text-left font-medium text-cyber-accent focus:outline-none"
        onClick={() => setIsOpen(!isOpen)}
      >
        <span>{title}</span>
        <span className={`transform transition-transform ${isOpen ? "rotate-180" : ""}`}>
          <svg width="10" height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1 1L5 5L9 1" stroke="#14b8a6" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </span>
      </button>
      {isOpen && (
        <div className="mt-4 flex flex-col gap-3">
          {options.map((opt) => (
            <label key={opt} className="flex items-center gap-3 cursor-pointer group">
              <div className="w-5 h-5 border border-gray-300 rounded-[4px] group-hover:border-cyber-accent flex items-center justify-center">
                {/* Checkbox state logic would go here */}
              </div>
              <span className="text-gray-500 group-hover:text-cyber-accent transition-colors">{opt}</span>
            </label>
          ))}
        </div>
      )}
